...
router.post("/", upload.single("image"), async (req, res) => {
  try {
    // const { id, name, price, category, available, demandScore, description } =
    //   req.body;
    const { name, price, description, category, isVeg } = req.body;
    ...
    // generate new ids in the backend if id does not exist
    // demand score is actually not needed here
    const values = [2173, name, price, category, 1, 90, realUrl, description];

    // Execute the database query
    await db.execute(query, values);

    // Send success response with the image URL
    res.status(200).json({ message: "Menu item saved successfully", realUrl });
  } catch (error) {
    console.error(error);
