// to be shown at the accountant panel
...
router.get('/', async (req, res) => {
    try{
     ...
     query = 'select * from active_order where status <> "paid" and status <> "declined" ';
      ...
      const transformOrders = (rows) => { //transforming fnctn
        ...
        rows.forEach(order => {
            const { order_id, user, itemName, price, quantity ,status,tableName} = order;
