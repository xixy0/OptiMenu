// get request for the orders to be seen at the cook's end
...
router.get('/', async (req, res) => {
    try{
     ...
     query = 'select * from active_order where status = "verified" or status = "approved"';
      ...
      const transformOrders = (rows) => { //transforming fnctn
        ...
        rows.forEach(order => {
            const { order_id, user, itemName, price, quantity ,status,note} = order;
